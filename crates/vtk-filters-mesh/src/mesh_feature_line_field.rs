...
pub fn ridge_direction_field(mesh: &PolyData, array_name: &str) -> PolyData {
    ...
    for i in 0..n{if nb[i].len()<2{dirs.extend_from_slice(&[0.0,0.0,0.0]);continue;}
        let p=mesh.points.get(i);let ni=nm[i];let vi=vals[i];
        let mut min_var=f64::INFINITY;let mut min_dir=[1.0,0.0,0.0];
        for &j in &nb[i]{let q=mesh.points.get(j);let d=[q[0]-p[0],q[1]-p[1],q[2]-p[2]];
            ...
            let tl=(tangent[0]*tangent[0]+tangent[1]*tangent[1]+tangent[2]*tangent[2]).sqrt();
            if tl<1e-15{continue;}
            let dv=(vals[j]-vi).abs()/tl;
            if dv<min_var{min_var=dv;min_dir=[tangent[0]/tl,tangent[1]/tl,tangent[2]/tl];}}
        dirs.extend_from_slice(&min_dir);}
    let mut r=mesh.clone();
    r.point_data_mut().add_array(AnyDataArray::F64(DataArray::from_vec("RidgeDir",dirs,3)));r
}
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let mut m=PolyData::from_triangles(
        ...
        let r=ridge_direction_field(&m,"h"); assert!(r.point_data().get_array("RidgeDir").is_some());
        assert_eq!(r.point_data().get_array("RidgeDir").unwrap().num_components(),3); } }
