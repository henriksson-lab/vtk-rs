...
pub fn normal_deviation(mesh: &PolyData) -> PolyData {
    ...
    let deviation: Vec<f64> = (0..n).map(|i| {
        if adj[i].is_empty() { return 0.0; }
        let ni = vnorm[i];
        let mut avg = [0.0f64; 3];
        for &j in &adj[i] { avg[0] += vnorm[j][0]; avg[1] += vnorm[j][1]; avg[2] += vnorm[j][2]; }
        let k = adj[i].len() as f64;
        avg[0] /= k; avg[1] /= k; avg[2] /= k;
        let al = (avg[0]*avg[0]+avg[1]*avg[1]+avg[2]*avg[2]).sqrt();
        if al < 1e-15 { return 0.0; }
        avg[0] /= al; avg[1] /= al; avg[2] /= al;
        let dot = ni[0]*avg[0]+ni[1]*avg[1]+ni[2]*avg[2];
        dot.clamp(-1.0, 1.0).acos() * 180.0 / std::f64::consts::PI
    }).collect();
    let mut result = mesh.clone();
    result.point_data_mut().add_array(AnyDataArray::F64(DataArray::from_vec("NormalDeviation", deviation, 1)));
    result.point_data_mut().set_active_scalars("NormalDeviation");
    result
}
