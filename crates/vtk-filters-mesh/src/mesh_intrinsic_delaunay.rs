...
pub fn intrinsic_delaunay(mesh: &PolyData, max_iterations: usize) -> PolyData {
    ...
    let nc=cells.len();
    for _ in 0..max_iterations{let mut flipped=false;
        ...
        for (&(ea,eb),faces) in &ef{if faces.len()!=2{continue;}
            ...
            if let (Some(va),Some(vb))=(va,vb){
                ...
                if new_min>old_min+0.01{
                    cells[faces[0]]=new_t1;cells[faces[1]]=new_t2;flipped=true;}}}
        if !flipped{break;}}
    let mut polys=CellArray::new();for c in &cells{polys.push_cell(c);}
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let m=PolyData::from_triangles(
        ...
        let r=intrinsic_delaunay(&m,10); assert_eq!(r.polys.num_cells(),2); } }
