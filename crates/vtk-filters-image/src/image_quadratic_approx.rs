...
pub fn image_quadratic_approx(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].abs(); if t<=0.5{0.75-t*t}else if t<=1.5{0.5*(t-1.5).powi(2)}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_quadratic_approx(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
