...
pub fn image_linear_congruential(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let x = buf[0].abs() as u64; ((x.wrapping_mul(1103515245).wrapping_add(12345)) >> 16) as f64 / 32768.0}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_linear_congruential(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
