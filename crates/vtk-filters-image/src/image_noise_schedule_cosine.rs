...
pub fn image_noise_schedule_cosine(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);((buf[0].clamp(0.0,1.0)+0.008)/(1.008)*std::f64::consts::FRAC_PI_2).cos().powi(2)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_noise_schedule_cosine(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
