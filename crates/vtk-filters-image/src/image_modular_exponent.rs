...
pub fn image_modular_exponent(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let x = buf[0].abs() as u64; let r = x.wrapping_pow(3) % 97; r as f64}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_modular_exponent(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
