...
pub fn image_wkb_approximation(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(2.0 * 9.109e-31 * buf[0].abs().max(1e-30)).sqrt() / 1.055e-34}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_wkb_approximation(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
