...
pub fn image_bubble_diameter(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.54 * 9.81f64.powf(-0.2) * (buf[0] - buf[0] * 0.01).max(0.0).powf(0.4) * buf[0].abs().max(0.01).powf(0.8)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_bubble_diameter(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
