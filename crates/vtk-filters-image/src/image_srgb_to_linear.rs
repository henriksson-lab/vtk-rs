...
pub fn image_srgb_to_linear(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0] <= 0.04045 { buf[0] / 12.92 } else { ((buf[0] + 0.055) / 1.055).powf(2.4) }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_srgb_to_linear(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
