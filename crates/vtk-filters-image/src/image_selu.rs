...
pub fn image_selu(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let a=1.6732632423543772;let l=1.0507009873554805; if buf[0]>0.0{l*buf[0]}else{l*a*(buf[0].exp()-1.0)} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_selu(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
