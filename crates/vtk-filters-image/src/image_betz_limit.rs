...
pub fn image_betz_limit(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.593 * 0.5 * 1.225 * buf[0].powi(3) * std::f64::consts::PI * 40.0f64.powi(2)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_betz_limit(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
