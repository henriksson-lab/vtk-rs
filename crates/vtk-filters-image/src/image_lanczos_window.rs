...
pub fn image_lanczos_window(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0]; if x.abs()<1e-15{1.0}else if x.abs()>3.0{0.0}else{let px=x*std::f64::consts::PI;(px.sin()/px)*((px/3.0).sin()/(px/3.0))} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_lanczos_window(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
