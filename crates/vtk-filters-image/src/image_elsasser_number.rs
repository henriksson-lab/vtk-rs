...
pub fn image_elsasser_number(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].powi(2) / (4e-7 * std::f64::consts::PI * 1000.0 * buf[0].abs().max(0.01) * 7.27e-5)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_elsasser_number(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
