...
pub fn image_schottky_barrier(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(1.6e-19 * buf[0].abs().max(1.0) / (4.0 * std::f64::consts::PI * 11.7 * 8.85e-12)).sqrt()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_schottky_barrier(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
