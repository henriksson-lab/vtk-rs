...
pub fn image_rayleigh_range(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);std::f64::consts::PI*buf[0].max(0.01).powi(2)/0.5}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_rayleigh_range(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
