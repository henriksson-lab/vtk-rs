...
pub fn image_orbital_inclination(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);2.0 * (6.674e-11 * 5.972e24 / buf[0].abs().max(1.0)).sqrt() * (buf[0] * std::f64::consts::PI / 360.0).sin()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_orbital_inclination(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
