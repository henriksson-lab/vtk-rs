...
pub fn image_journal_bearing_load(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0] * 0.001 * 30.0 * 0.05 * 0.1 / (0.001f64.powi(2))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_journal_bearing_load(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
