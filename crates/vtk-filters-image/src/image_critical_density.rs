...
pub fn image_critical_density(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);3.0 * (70.0f64 / 3.086e22).powi(2) / (8.0 * std::f64::consts::PI * 6.674e-11)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_critical_density(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
