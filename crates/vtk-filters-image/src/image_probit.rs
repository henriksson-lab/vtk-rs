...
pub fn image_probit(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let p=buf[0].clamp(0.001,0.999); 5.5556*(1.0-(p/(1.0-p)).powf(-0.1186)) }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_probit(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
