...
pub fn image_b_spline_cubic(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].abs(); if t<=1.0{(3.0*t*t*t-6.0*t*t+4.0)/6.0}else if t<=2.0{(2.0-t).powi(3)/6.0}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_b_spline_cubic(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
