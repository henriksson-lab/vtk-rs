...
pub fn image_posterize(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i| { arr.tuple_as_f64(i, &mut buf); { let v = buf[0]; (v * 4.0).floor() / 3.0 } }).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test_image_posterize() {
        let img = ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r = image_posterize(&img, "v"); assert_eq!(r.dimensions(), [5, 5, 1]);
    }
}
