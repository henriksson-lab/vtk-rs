...
pub fn image_reflectance_thin_film(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);((1.5f64 - 1.0) / (1.5 + 1.0)).powi(2) + 4.0 * 1.5 * 1.0 / (1.5f64 + 1.0).powi(2) * (std::f64::consts::PI * buf[0] * 1.5 / 550e-9).sin().powi(2)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_reflectance_thin_film(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
