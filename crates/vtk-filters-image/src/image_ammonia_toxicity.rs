...
pub fn image_ammonia_toxicity(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);1.0 / (1.0 + 10.0f64.powf(0.09 * (buf[0] - 25.0) + 9.24 - 8.0))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_ammonia_toxicity(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
