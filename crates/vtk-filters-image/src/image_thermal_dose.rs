...
pub fn image_thermal_dose(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0] * if buf[0] > 43.0 { 2.0f64.powf(buf[0] - 43.0) } else { 2.0f64.powf((buf[0] - 43.0) / 4.0) }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_thermal_dose(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
