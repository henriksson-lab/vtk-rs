...
pub fn image_longshore_transport(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].powi(2) * (2.0 * buf[0] * std::f64::consts::PI / 180.0).sin() * 0.39}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_longshore_transport(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
