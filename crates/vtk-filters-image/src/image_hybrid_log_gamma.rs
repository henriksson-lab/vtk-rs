...
pub fn image_hybrid_log_gamma(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0]<=1.0/12.0{(3.0*buf[0]).sqrt()}else{0.17883277*(buf[0]*12.0-0.28466892).ln()+0.55991073}}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_hybrid_log_gamma(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
