...
pub fn image_equilibrium_constant(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(-buf[0]*1000.0/(8.314*300.0)).exp()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_equilibrium_constant(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
