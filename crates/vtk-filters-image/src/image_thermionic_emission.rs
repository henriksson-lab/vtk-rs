...
pub fn image_thermionic_emission(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);120.0 * buf[0].abs().max(1.0).powi(2) * (-1.6e-19 * 0.7 / (1.381e-23 * buf[0].abs().max(1.0))).exp()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_thermionic_emission(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
