...
pub fn image_jeans_mass(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(5.0 * 1.381e-23 * buf[0].abs().max(1.0) / (6.674e-11 * 1.67e-27)).powf(1.5) / (buf[0].abs().max(1e-20)).sqrt()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_jeans_mass(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
