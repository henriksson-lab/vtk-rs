...
pub fn image_power_law(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0].abs()>0.01{buf[0].abs().powf(-2.5)*buf[0].signum()}else{0.0}}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_power_law(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
