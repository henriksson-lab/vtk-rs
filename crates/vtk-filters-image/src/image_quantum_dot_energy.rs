...
pub fn image_quantum_dot_energy(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);std::f64::consts::PI.powi(2) * 1.055e-34f64.powi(2) / (2.0 * 0.067 * 9.109e-31 * buf[0].abs().max(1e-10).powi(2))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_quantum_dot_energy(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
