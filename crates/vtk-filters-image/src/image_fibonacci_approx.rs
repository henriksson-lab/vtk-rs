...
pub fn image_fibonacci_approx(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let phi = (1.0 + 5.0f64.sqrt()) / 2.0; (phi.powf(buf[0].abs()) / 5.0f64.sqrt()).round()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_fibonacci_approx(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
