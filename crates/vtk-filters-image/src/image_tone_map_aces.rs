...
pub fn image_tone_map_aces(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let x=buf[0]; (x*(2.51*x+0.03))/(x*(2.43*x+0.59)+0.14)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_tone_map_aces(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
