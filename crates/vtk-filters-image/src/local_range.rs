...
pub fn image_local_range(input: &ImageData, scalars: &str, radius: usize) -> ImageData {
    ...
    for k in 0..nz{for j in 0..ny{for i in 0..nx{
        ...
        for dk in -r..=r{for dj in -r..=r{for di in -r..=r{
            ...
            let v=values[kk*ny*nx+jj*nx+ii]; lo=lo.min(v); hi=hi.max(v);
        }}}
        result[k*ny*nx+j*nx+i]=hi-lo;
    }}}

    let mut img=input.clone();
    img.point_data_mut().add_array(AnyDataArray::F64(DataArray::from_vec("LocalRange", result, 1)));
    img
}
...
pub fn image_local_std(input: &ImageData, scalars: &str, radius: usize) -> ImageData {
    ...
    for k in 0..nz{for j in 0..ny{for i in 0..nx{
        ...
        result[k*ny*nx+j*nx+i]=(sum2/cnt as f64-mean*mean).max(0.0).sqrt();
    }}}

    let mut img=input.clone();
    img.point_data_mut().add_array(AnyDataArray::F64(DataArray::from_vec("LocalStd", result, 1)));
    img
}
...
mod tests {
    ...
    fn edge_high_range() {
        ...
        let mut buf=[0.0f64];
        arr.tuple_as_f64(1,&mut buf); assert_eq!(buf[0],100.0); // edge
    ...
    fn uniform_zero_std() {
        ...
        let mut buf=[0.0f64];
        for i in 0..9{arr.tuple_as_f64(i,&mut buf);assert!(buf[0]<1e-10);}
    }
