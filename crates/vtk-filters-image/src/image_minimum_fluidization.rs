...
pub fn image_minimum_fluidization(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].abs().max(1e-6).powi(2) * (2650.0 - 1000.0) * 9.81 / (150.0 * 0.001 * (1.0 - 0.4))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_minimum_fluidization(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
