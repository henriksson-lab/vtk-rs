...
pub fn image_pmv_thermal_comfort(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.303 * (-0.036 * buf[0]).exp() * (buf[0] - 58.15) + 0.028}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_pmv_thermal_comfort(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
