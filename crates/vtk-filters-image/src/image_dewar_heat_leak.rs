...
pub fn image_dewar_heat_leak(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);5.67e-8 * (buf[0].powi(4) - 77.0f64.powi(4)) * 0.05}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_dewar_heat_leak(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
