...
pub fn image_hawking_radiation(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].powi(3)/((buf[0]*10.0).exp()-1.0).max(1e-30)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_hawking_radiation(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
