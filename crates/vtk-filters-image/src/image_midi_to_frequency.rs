...
pub fn image_midi_to_frequency(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);440.0 * 2.0f64.powf((buf[0] - 69.0) / 12.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_midi_to_frequency(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
