...
pub fn image_silverman_kernel(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0].abs()/std::f64::consts::SQRT_2; 0.5*(-x).exp()*(x).sin().abs().max(0.001) }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_silverman_kernel(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
