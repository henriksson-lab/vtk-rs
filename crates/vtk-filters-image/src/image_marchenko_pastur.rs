...
pub fn image_marchenko_pastur(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0]>0.01&&buf[0]<4.0{((4.0-buf[0])*buf[0]).max(0.0).sqrt()/(2.0*std::f64::consts::PI*buf[0])}else{0.0}}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_marchenko_pastur(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
