...
pub fn image_heart_rate_variability(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);60.0+10.0*(buf[0]*0.1).sin()+5.0*(buf[0]*0.25).sin()+2.0*(buf[0]*1.0).sin()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_heart_rate_variability(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
