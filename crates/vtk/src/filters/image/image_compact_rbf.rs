...
pub fn image_compact_rbf(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0].abs()<=1.0{let t=1.0-buf[0].abs();t.powi(4)*(4.0*buf[0].abs()+1.0)}else{0.0}}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_compact_rbf(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
