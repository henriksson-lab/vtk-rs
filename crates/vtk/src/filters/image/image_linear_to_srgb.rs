...
pub fn image_linear_to_srgb(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0] <= 0.0031308 { buf[0] * 12.92 } else { 1.055 * buf[0].powf(1.0/2.4) - 0.055 }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_linear_to_srgb(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
