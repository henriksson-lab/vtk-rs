...
pub fn image_breakwater_stability(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0] * 2650.0 / (2.0 * (2650.0f64 / 1025.0 - 1.0).powi(3) * (buf[0] * std::f64::consts::PI / 180.0).tan())}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_breakwater_stability(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
