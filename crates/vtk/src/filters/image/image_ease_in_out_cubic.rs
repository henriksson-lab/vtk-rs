...
pub fn image_ease_in_out_cubic(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].clamp(0.0,1.0); if t<0.5{4.0*t*t*t}else{1.0-(-2.0*t+2.0).powi(3)/2.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_ease_in_out_cubic(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
