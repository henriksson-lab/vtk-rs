...
pub fn image_penstock_head_loss(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.02 * buf[0].abs().max(0.01) * buf[0].powi(2) / (2.0 * 9.81 * 1.0f64.powi(5))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_penstock_head_loss(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
