...
pub fn image_hall_conductance(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].round() * 1.6e-19f64.powi(2) / 6.626e-34}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_hall_conductance(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
