...
pub fn image_magnitude_scaling(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);10.0f64.powf(2.24) / (10.0f64.powf(2.56 * 0.34 * buf[0]))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_magnitude_scaling(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
