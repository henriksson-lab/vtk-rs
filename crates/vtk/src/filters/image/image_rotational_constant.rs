...
pub fn image_rotational_constant(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);1.055e-34 / (8.0 * std::f64::consts::PI.powi(2) * 1e-26 * buf[0].abs().max(1e-10).powi(2))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_rotational_constant(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
