...
pub fn image_collatz_step(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let x = buf[0].abs().round(); if x % 2.0 == 0.0 { x / 2.0 } else { 3.0 * x + 1.0 }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_collatz_step(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
