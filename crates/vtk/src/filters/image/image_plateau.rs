...
pub fn image_plateau(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0].abs(); if x<0.3{1.0}else if x<0.7{0.5+0.5*((x-0.3)/0.4*std::f64::consts::PI).cos()}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_plateau(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
