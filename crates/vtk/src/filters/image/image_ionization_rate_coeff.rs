...
pub fn image_ionization_rate_coeff(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);1e-14 * (-15.76 / (buf[0].abs().max(0.01) * 1.381e-23 / 1.6e-19)).exp()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_ionization_rate_coeff(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
