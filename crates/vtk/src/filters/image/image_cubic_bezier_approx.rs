...
pub fn image_cubic_bezier_approx(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].clamp(0.0,1.0); 3.0*(1.0-t)*(1.0-t)*t*0.25+3.0*(1.0-t)*t*t*0.75+t*t*t }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_cubic_bezier_approx(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
