...
pub fn image_diffraction(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0]*std::f64::consts::PI; if x.abs()<1e-15{1.0}else{(x.sin()/x).powi(2)} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_diffraction(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
