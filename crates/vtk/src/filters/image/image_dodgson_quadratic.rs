...
pub fn image_dodgson_quadratic(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].abs(); if t<=0.5{-2.0*t*t+1.0}else if t<=1.5{t*t-3.0*t+2.25-0.25}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_dodgson_quadratic(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
