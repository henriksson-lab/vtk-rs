...
pub fn image_airy_disk(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0].abs().max(1e-15)*std::f64::consts::PI; let j1=x.sin()/x-x.cos()/(x*x); (2.0*j1/x.max(1e-15)).powi(2).min(1.0) }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_airy_disk(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
