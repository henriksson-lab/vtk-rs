...
pub fn image_fuzzy_trapezoidal(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let a=-2.0f64; let b=-1.0f64; let c=1.0f64; let d=2.0f64; ((buf[0]-a)/(b-a)).min(1.0).min((d-buf[0])/(d-c)).max(0.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_fuzzy_trapezoidal(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
