...
pub fn image_thermal_duty(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].abs().max(0.01) * 4180.0 * (buf[0] - 30.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_thermal_duty(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
