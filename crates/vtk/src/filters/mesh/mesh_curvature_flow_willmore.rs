...
pub fn willmore_flow(mesh: &PolyData, steps: usize, dt: f64) -> PolyData {
    ...
    for _ in 0..steps{let prev=pos.clone();
        ...
        for i in 0..n{if nb[i].is_empty(){continue;}let k=nb[i].len() as f64;
            for &j in &nb[i]{bilap[i][0]+=h_vec[j][0]-h_vec[i][0];bilap[i][1]+=h_vec[j][1]-h_vec[i][1];bilap[i][2]+=h_vec[j][2]-h_vec[i][2];}
            bilap[i][0]/=k;bilap[i][1]/=k;bilap[i][2]/=k;}
        // Willmore gradient: move along -Lap(H)*n direction
        for i in 0..n{let hn=bilap[i][0]*nm[i][0]+bilap[i][1]*nm[i][1]+bilap[i][2]*nm[i][2];
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let m=PolyData::from_triangles(
        ...
        let r=willmore_flow(&m,3,0.01); assert_eq!(r.points.len(),4); } }
