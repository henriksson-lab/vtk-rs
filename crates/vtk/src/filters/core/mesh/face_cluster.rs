...
pub fn face_cluster_by_normal(input: &PolyData, angle_threshold_deg: f64) -> PolyData {
    ...
    for faces in edge_faces.values(){if faces.len()==2{adj[faces[0]].push(faces[1]);adj[faces[1]].push(faces[0]);}}
    ...
    for seed in 0..nc {
        ...
        labels[seed]=current; queue.push_back(seed);

        while let Some(fi)=queue.pop_front(){
            for &ni in &adj[fi]{
                if labels[ni]!=0{continue;}
                let dot=normals[fi][0]*normals[ni][0]+normals[fi][1]*normals[ni][1]+normals[fi][2]*normals[ni][2];
                if dot>=cos_thresh{labels[ni]=current;queue.push_back(ni);}
            }
        }
    }
    ...
    pd.cell_data_mut().add_array(AnyDataArray::F64(DataArray::from_vec("FaceCluster", labels_f, 1)));
    pd
}
