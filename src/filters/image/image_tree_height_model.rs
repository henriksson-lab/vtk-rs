...
pub fn image_tree_height_model(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);30.0 * (1.0 - (-0.05 * buf[0]).exp()).powf(1.5)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_tree_height_model(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
