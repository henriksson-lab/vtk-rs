...
pub fn image_kaiser_approx(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0].clamp(-1.0,1.0); let a=3.0*std::f64::consts::PI; let v=(1.0-x*x).max(0.0).sqrt()*a; if v.abs()<1e-15{1.0}else{v.sin()/v} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_kaiser_approx(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
