...
pub fn image_compand(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let mu=255.0f64; mu.ln_1p().recip() as f64*(1.0+mu*buf[0].abs()).ln()*buf[0].signum() }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_compand(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
