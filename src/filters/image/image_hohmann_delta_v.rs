...
pub fn image_hohmann_delta_v(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(6.674e-11 * 5.972e24 / buf[0].abs().max(1.0)).sqrt() * ((2.0 * buf[0] * 2.0 / (buf[0] + buf[0] * 2.0)).sqrt() - 1.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_hohmann_delta_v(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
