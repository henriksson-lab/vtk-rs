...
pub fn image_membrane_conductivity(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.1 * (buf[0].clamp(0.0, 1.0)).powf(1.5)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_membrane_conductivity(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
