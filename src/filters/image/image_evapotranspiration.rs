...
pub fn image_evapotranspiration(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.0023 * (buf[0] + 17.8) * 10.0f64.sqrt() * 15.0}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_evapotranspiration(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
