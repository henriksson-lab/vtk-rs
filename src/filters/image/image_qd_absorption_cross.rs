...
pub fn image_qd_absorption_cross(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);buf[0].abs().max(1e-9).powi(3) * 1e6}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_qd_absorption_cross(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
