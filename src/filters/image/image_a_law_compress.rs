...
pub fn image_a_law_compress(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let a=87.6f64; let x=buf[0].abs(); if x<1.0/a{a*x/(1.0+a.ln())*buf[0].signum()}else{(1.0+(a*x).ln())/(1.0+a.ln())*buf[0].signum()} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_a_law_compress(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
