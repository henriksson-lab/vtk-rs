...
pub fn image_dlvo_interaction(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);64.0 * 1.381e-23 * 300.0 * 1e18 * (buf[0] * 0.01).tanh().powi(2) * (-1e8 * buf[0]).exp() / 1e8}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_dlvo_interaction(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
