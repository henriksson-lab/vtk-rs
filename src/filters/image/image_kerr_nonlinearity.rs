...
pub fn image_kerr_nonlinearity(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);2.0 * std::f64::consts::PI * 3e-20 * buf[0] * 0.01 / (1.064e-6 * buf[0].abs().max(1e-10))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_kerr_nonlinearity(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
