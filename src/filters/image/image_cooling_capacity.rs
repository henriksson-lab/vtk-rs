...
pub fn image_cooling_capacity(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);200e-6 * buf[0].abs().max(0.01) * buf[0] - 0.5 * buf[0].powi(2) * 1e-5 - 1.5 * (buf[0] - 300.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_cooling_capacity(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
