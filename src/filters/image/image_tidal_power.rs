...
pub fn image_tidal_power(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.5 * 1025.0 * buf[0].abs().powi(3) * std::f64::consts::PI * 5.0f64.powi(2)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_tidal_power(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
