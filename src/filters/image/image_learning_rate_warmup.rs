...
pub fn image_learning_rate_warmup(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);if buf[0]<0.1{buf[0]*10.0}else{0.5*(1.0+((buf[0]-0.1)/(0.9)*std::f64::consts::PI).cos())}}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_learning_rate_warmup(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
