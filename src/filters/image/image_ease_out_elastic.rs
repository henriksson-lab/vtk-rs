...
pub fn image_ease_out_elastic(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].clamp(0.0,1.0); if t<=0.0||t>=1.0{t}else{(2.0f64).powf(-10.0*t)*((t*10.0-0.75)*std::f64::consts::TAU/3.0).sin()+1.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_ease_out_elastic(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
