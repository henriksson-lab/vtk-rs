...
pub fn image_wind_chill(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);13.12 + 0.6215 * buf[0] - 11.37 * 5.0f64.powf(0.16) + 0.3965 * buf[0] * 5.0f64.powf(0.16)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_wind_chill(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
