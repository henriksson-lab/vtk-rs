...
pub fn image_gaussian_error_linear(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);0.5*buf[0]*(1.0+(buf[0]*std::f64::consts::FRAC_2_SQRT_PI*0.5*(buf[0]*buf[0]*(-0.5)).exp()).min(1.0))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_gaussian_error_linear(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
