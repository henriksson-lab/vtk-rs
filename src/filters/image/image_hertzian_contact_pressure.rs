...
pub fn image_hertzian_contact_pressure(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(6.0 * buf[0] * 200e9f64.powi(2) / (std::f64::consts::PI.powi(3) * 0.01f64.powi(2))).powf(1.0/3.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_hertzian_contact_pressure(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
