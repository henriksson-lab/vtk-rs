...
pub fn image_evanescent_decay(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);(-buf[0] * ((std::f64::consts::PI / 0.01).powi(2) - buf[0].powi(2)).max(0.0).sqrt()).exp()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_evanescent_decay(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
