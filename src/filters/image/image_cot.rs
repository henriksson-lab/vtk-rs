...
pub fn image_cot(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let s=buf[0].sin();let c=buf[0].cos(); if s.abs()>1e-15{c/s}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_cot(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
