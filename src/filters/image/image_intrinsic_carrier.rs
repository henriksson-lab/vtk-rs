...
pub fn image_intrinsic_carrier(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);5.2e15 * buf[0].abs().max(1.0).powf(1.5) * (-6300.0 / buf[0].abs().max(1.0)).exp()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_intrinsic_carrier(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
