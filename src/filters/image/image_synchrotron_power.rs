...
pub fn image_synchrotron_power(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);1.6e-19f64.powi(4) * buf[0].powi(2) * 1.0f64.powi(2) / (6.0 * std::f64::consts::PI * 8.85e-12 * 9.109e-31f64.powi(4) * 3e8f64.powi(3))}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_synchrotron_power(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
