...
pub fn image_schaum_3(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let t=buf[0].abs(); if t<=0.5{1.0-t*t*2.0}else if t<=1.5{(t-1.5)*(t-0.5)}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_schaum_3(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
