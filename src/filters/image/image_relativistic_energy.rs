...
pub fn image_relativistic_energy(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);9.109e-31 * 3e8f64.powi(2) * (1.0 / (1.0 - (buf[0] / 3e8).powi(2)).max(1e-10).sqrt() - 1.0)}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_relativistic_energy(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
