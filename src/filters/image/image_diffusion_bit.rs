...
pub fn image_diffusion_bit(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);let x = buf[0].to_bits(); (x ^ (x >> 1)).count_ones() as f64 / 64.0}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_diffusion_bit(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
