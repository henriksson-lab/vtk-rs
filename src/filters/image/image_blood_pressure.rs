...
pub fn image_blood_pressure(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);80.0+40.0*(-((buf[0].rem_euclid(1.0)-0.15)*15.0).powi(2)).exp()+10.0*(-((buf[0].rem_euclid(1.0)-0.4)*10.0).powi(2)).exp()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_blood_pressure(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
