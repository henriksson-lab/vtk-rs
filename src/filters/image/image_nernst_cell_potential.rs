...
pub fn image_nernst_cell_potential(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);1.1 - 0.0592 / 2.0 * buf[0].abs().max(0.001).log10()}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_nernst_cell_potential(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
