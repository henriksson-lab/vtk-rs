...
pub fn image_parzen_window(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n).map(|i|{arr.tuple_as_f64(i,&mut buf);{ let x=buf[0].abs(); if x<=0.5{1.0-6.0*x*x+6.0*x*x*x}else if x<=1.0{2.0*(1.0-x).powi(3)}else{0.0} }}).collect();
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let img=ImageData::from_function([5,5,1],[1.0,1.0,1.0],[0.0,0.0,0.0],"v",|x,_,_|x+1.0);
        let r=image_parzen_window(&img,"v"); assert_eq!(r.dimensions(),[5,5,1]); } }
